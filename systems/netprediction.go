...
// Reconcile compares the server's authoritative position with the predicted
// position for the acknowledged sequence. If the error exceeds the configured
// threshold, it corrects the local position and replays unacknowledged inputs.
// Returns true if a correction was applied.
func (p *NetPrediction) Reconcile(
	serverPos *netcomponents.NetPositionData,
	serverVel *netcomponents.NetVelocityData,
	lastSeq uint32,
) bool {
	if lastSeq == 0 {
		return false
	...
	// Update ground state from server
	p.OnGround = math.Abs(serverVel.SpeedY) < 0.1

	predErr := p.Buffer.PredictionError(lastSeq, serverPos.X, serverPos.Y)

	if predErr <= cfg.Netcode.SmoothThreshold {
		// Tiny error — don't reset velocity, prediction is essentially correct
		return false
	}

	if predErr > cfg.Netcode.SnapThreshold {
		// Large error — snap to server position, hard reset velocity
		p.VelX = serverVel.SpeedX
		p.VelY = serverVel.SpeedY
	} else {
		// Medium error — lerp toward server, blend velocity
		record, ok := p.Buffer.Get(lastSeq)
		if !ok {
			return false
		}
		corrX := record.PredictedX + (serverPos.X-record.PredictedX)*cfg.Netcode.SmoothFactor
		corrY := record.PredictedY + (serverPos.Y-record.PredictedY)*cfg.Netcode.SmoothFactor
		serverPos.X = corrX
		serverPos.Y = corrY

		// Blend velocity instead of hard reset
		p.VelX += (serverVel.SpeedX - p.VelX) * cfg.Netcode.VelocityBlendRate
		p.VelY += (serverVel.SpeedY - p.VelY) * cfg.Netcode.VelocityBlendRate
	}

	// Sync resolv object to corrected position
	if p.PlayerObj != nil {
		p.PlayerObj.X = serverPos.X
		p.PlayerObj.Y = serverPos.Y
		p.PlayerObj.Update()
	}

	// Replay unacknowledged inputs on top of corrected position
	unacked := p.Buffer.GetUnacknowledged(lastSeq)
	for _, record := range unacked {
		p.replayStep(record.Input, serverPos)
		p.Buffer.Store(record.Input, serverPos.X, serverPos.Y)
	}

	return true
}

// replayStep re-applies a single input step during reconciliation.
func (p *NetPrediction) replayStep(input messages.PlayerInput, pos *netcomponents.NetPositionData) {
	if input.Direction != 0 {
		p.VelX += float64(input.Direction) * predAcceleration
	}
	...
	// Use collision space if available
	if p.PlayerObj != nil {
		p.PlayerObj.X = pos.X
		p.PlayerObj.Y = pos.Y
		p.PlayerObj.Update()
